   ...
   if ($rowcount > 0)
   {
      ...
      for ($zz=1;$zz<$countfield-1;$zz++)
      {
         ...
         $tsql .= $datafield[$zz] . "=" . "'" . $varvalue . "',";
   		...
   		$tsql = substr($tsql,0,strlen($tsql)-1) .	" WHERE $datafield[0]='$prikey'";
   }
   else
   {
...
<?
   ...
   if(sqlsrv_has_rows($sqlConn))
   {
      ...
      {
?>
   	  	      			         <option value='D<? echo $row[$datafield[0]]; ?>'><? echo $row[$datafield[0]]; ?> - <? echo $row[$datafield[1]]; ?> - <? echo $row[$datafield[2]]; ?></option>
<?
      }
   }
   sqlsrv_free_stmt( $sqlConn );
