...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM Tbl_KodeProduk
   					WHERE produk_loan_type='$thecode'";
...
function SAVETABLE()
{
  ...
  $userpwd=$_POST['userpwd'];
   ...
   $cttext=$_POST['cttext'];
   $tdtext=$_POST['tdtext'];
   $minratetext=floatval($_POST['minratetext']);
   $maxratetext=floatval($_POST['maxratetext']);
   $rfloortext=floatval($_POST['rfloortext']);
   $ratectext=floatval($_POST['ratectext']);
   $ftext=$_POST['ftext'];
   $atext=$_POST['atext'];
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM Tbl_KodeProduk
   					WHERE produk_loan_type='$tltext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE Tbl_KodeProduk SET produk_currency_type = '$cttext',
											produk_type_description = '$tdtext',
											produk_minimum_int_rate = '$minratetext',
											produk_maximum_int_rate = '$maxratetext',
											produk_rate_floor = '$rfloortext',
											produk_rate_ceiling = '$ratectext',
											produk_create_userid = '$userid',
											produk_create_time = '$dtext',
											produk_flag = '$ftext',
											produk_active = '$atext'
											where produk_loan_type = '$tltext'";
   }
   ...
   {
      $tsql = "INSERT INTO Tbl_KodeProduk VALUES('$tltext','$cttext','$tdtext','$minratetext','$maxratetext','$rfloortext','$ratectext','$userid','$dtext','$ftext','$atext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./Tbl_KodeProduk.php";
        ...
        function cekSave()
        {
			var vcode = document.formsubmit.tltext;
			var vname = document.formsubmit.cttext;
			var vdesc = document.formsubmit.tdtext;
			var vmin = document.formsubmit.minratetext;
			var vmax = document.formsubmit.maxratetext;
			var vrf = document.formsubmit.rfloortext;
			var vrc = document.formsubmit.ratectext;
			
			if(vcode.value.trim().length == 0){
				 alert("Loan Type harus diisi ");
				 vcode.focus();
				 return false;
			}
			else if(vname.value.trim().length == 0){
				 alert("Currency Type harus diisi ");
				 vname.focus();
				 return false;
			}
			else if(vdesc.value.trim().length == 0){
				 alert("Type Desc harus diisi ");
				 vdesc.focus();
				 return false;
			}
			else if(vmin.value.trim().length == 0){
				 alert("Min Int Rate harus diisi ");
				 vmin.focus();
				 return false;
			}
			else if(vmax.value.trim().length == 0){
				 alert("Max In Rate harus diisi ");
				 vname.focus();
				 return false;
			}
			else if(vrf.value.trim().length == 0){
				 alert("Rate Floor harus diisi ");
				 vrf.focus();
				 return false;
			}
			else if(vrc.value.trim().length == 0){
				 alert("Rate Ceiling harus diisi ");
				 vrc.focus();
				 return false;
			}
			else{
...
<?
	...
	$tsql = "SELECT * FROM Tbl_KodeProduk
					WHERE produk_loan_type='$thekey'";
	$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
	$params = array(&$_POST['query']);
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			$ttloan = $row['produk_loan_type'];
			$tctype = $row['produk_currency_type'];
			$ttdesc = $row['produk_type_description'];
			$tminir = $row['produk_minimum_int_rate'];
			$tmaxir = $row['produk_maximum_int_rate'];
			$trfloor = $row['produk_rate_floor'];
			$tratec = $row['produk_rate_ceiling'];
			$tflag = $row['produk_flag'];
			$tactive = $row['produk_active'];
		}
	}
	//echo $tcode;
				...
				<td width="100%" style="padding-bottom:10px;padding-top:10px;">
					<table border="0">
						<tr>
							...
							<td>
								: <input type="text" name="tltext" id="tltext" value="<?=$ttloan?>" style="width:50px;margin-right:10px;">
							</td>
							<td>
								Currency Type 
							</td>
							<td>
							: <input type="text" name="cttext" id="cttext" value="<?=$tctype?>" style="width:50px;margin-right:10px;">
							</td>
							<td>
								Type Desc
							</td>
							<td>
								: <input type="text" name="tdtext" id="tdtext"  value="<?=$ttdesc?>"style="width:100px;margin-right:10px;">
							</td>
							<td>
								Min Int Rate 
							</td>
							<td>
								: <input type="text" name="minratetext" id="minratetext"  value="<?=$tminir?>"style="width:50px;margin-right:10px;">
							</td>
						</tr>
						<tr>
							<td>
							...
							<td>
								: <input type="text" name="maxratetext" id="maxratetext" value="<?=$tmaxir?>" style="width:50px;margin-right:10px;">
							</td>
							<td>
								Rate Floor 
							</td>
							<td>
								: <input type="text" name="rfloortext" id="rfloortext" value="<?=$trfloor?>" style="width:50px;margin-right:10px;">
							</td>
							<td>
								Rate Ceiling
							</td>
							<td>
								: <input type="text" name="ratectext" id="ratectext" value="<?=$tratec?>" style="width:100px;margin-right:10px;">
							</td>
							<td>
								Flag 
							</td>
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="54px">Loan Type</th>
										<th width="108px">Currency Type</th>
										<th width="114px">Type Desc</th>
										<th width="162px">Min Int Rate</th>
										<th width="54px">Max Int Rate</th>
										<th width="87px">Rate Floor</th>
										<th width="54px">Rate Ceiling</th>
										<th width="54px">Flag</th>
										<th width="55px">Active</th>
										<th width="110px"></th>
									</tr>
								</table>
								</div>
...
?>
									<tr align="center" style="line-height:20px">
										...
										<td><?=$row[1]?></td>
										<td><?=$row[2]?></td>
										<td><?=floatval($row[3])?></td>
										<td><?=floatval($row[4])?></td>
										<td><?=floatval($row[5])?></td>
										<td><?=floatval($row[6])?></td>
										<td><?=$row[9]?></td>
										<td><?=$row[10]?></td>
										<td>
