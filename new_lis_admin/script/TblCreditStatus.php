...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM TblCreditStatus
   					WHERE status_code='$thecode'";
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM TblCreditStatus
   					WHERE status_code='$ctext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE TblCreditStatus SET status_name = '$ntext',
											status_flag = '$ftext',
											status_active = '$atext',
											status_create_userid = '$userid',
											status_create_time = '$dtext'
											where status_code = '$ctext'";
   }
   else
   {
      $tsql = "INSERT INTO TblCreditStatus VALUES('$ctext','$ntext','$ftext','$atext','$userid','$dtext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./TblCreditStatus.php";
...
<?
	...
	$tsql = "SELECT * FROM TblCreditStatus
					WHERE status_code='$thekey'";
	$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
	$params = array(&$_POST['query']);
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			$tcode = $row['status_code'];
			$tname = $row['status_name'];
			$tflag = $row['status_flag'];
			$tactive = $row['status_active'];
		}
	}
	//echo $tcode;
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="130px">Credit Status Code</th>
										<th width="193px">Credit Status Name</th>
										<th width="140px">Credit Status Flag</th>
										<th width="129px">Credit Status Active</th>
										<th width="135px"></th>
									</tr>
								</table>
								</div>
