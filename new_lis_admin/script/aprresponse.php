...
?>
	...
	<table width = "500" align = "center" border = "0">
		...
		<?php
			$tsqlb = "SELECT * FROM tbl_COL_App where ap_lisregno = '$Custnomid' and ap_status = 'A03'";

			$ab = sqlsrv_query($conn, $tsqlb);

			if ( $ab === false)
				die( FormatErrors( sqlsrv_errors() ) );

			if(sqlsrv_has_rows($ab))
			{ 
			//-----
		?>
			<?php
			$asql = "SELECT * FROM Tbl_CustomerFlag where custnomid = '$Custnomid' and custflagapr = 'Z'";
			...
			<td width = "60%" style="padding-left : 10px;">
				<select name="actions" id="actions" onchange="popOrNot(this,0)">
					...
					<?
						...
						{ 
					?>
					<option value="_blank:./viewaprresponse.php?custnomid=&custjeniscol=<? echo $row['cust_jeniscol'];?>"><? echo $row['cust_jeniscol']; ?></option>
					<?			
						}
			...
			<td width = "60%" style="padding-left : 10px;">
				<select name="actions" id="actions" onchange="popOrNot(this,0)">
					...
					<?
						...
						{ 
					?>
					<option value="_blank:./viewaprresponse.php?custnomid=<? echo $Custnomid; ?>&custjeniscol=<? echo $row['cust_jeniscol'];?>"><? echo $row['cust_jeniscol']; ?></option>
					<?			
						}
