...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM Tbl_CreditNeed
   					WHERE credit_need_code='$thecode'";
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM Tbl_CreditNeed
   					WHERE credit_need_code='$ctext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE Tbl_CreditNeed SET credit_need_name = '$ntext',
											credit_need_flag = '$ftext',
											credit_need_active = '$atext',
											credit_need_create_userid = '$userid',
											credit_need_create_time = '$dtext'
											where credit_need_code = '$ctext'";
   }
   else
   {
      $tsql = "INSERT INTO Tbl_CreditNeed VALUES('$ctext','$ntext','$ftext','$atext','$userid','$dtext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./Tbl_CreditNeed.php";
...
<?
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			$tcode = $row['credit_need_code'];
			$tname = $row['credit_need_name'];
			$tflag = $row['credit_need_flag'];
			$tactive = $row['credit_need_active'];
		}
	}
	//echo $tcode;
   	      ...
   	      <table width="760" border=0 cellpadding=0 cellspacing=0>
			  ...
			  <tr>
				<td style="padding-bottom:10px;padding-top:10px;">
					Code : <input type="text" name="ctext" id="ctext" value="<?=$tcode?>" style="width:70px;margin-right:20px;">
					Name : <input type="text" name="ntext" id="ntext" value="<?=$tname?>" style="width:130px;margin-right:20px;">
					Flag : <input type="text" name="ftext" id="ftext" value="<?=$tflag?>" style="width:70px;margin-right:20px;">
					Active : <input type="text" name="atext" id="atext"  value="<?=$tactive?>"style="width:70px;margin-right:20px;">
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="130px">Credit Need Code</th>
										<th width="193px">Credit Need Name</th>
										<th width="140px">Credit Need Flag</th>
										<th width="129px">Credit Need Active</th>
										<th width="135px"></th>
									</tr>
								</table>
								</div>
...
<?
									$strsql="select * from Tbl_CreditNeed";
									$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
