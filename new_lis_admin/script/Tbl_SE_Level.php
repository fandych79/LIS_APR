...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM Tbl_SE_Level
   					WHERE level_code='$thecode'";
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM Tbl_SE_Level
   					WHERE level_code='$ctext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE Tbl_SE_Level SET level_name = '$ntext',
											level_flag = '$ftext'
											where level_code = '$ctext'";
   }
   else
   {
      $tsql = "INSERT INTO Tbl_SE_Level VALUES('$ctext','$ntext','$ftext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./Tbl_SE_Level.php";
...
<?
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			$tcode = $row['level_code'];
			$tname = $row['level_name'];
			$tflag = $row['level_flag'];
		}
	}
	//echo $tcode;
   	      ...
   	      <table width="760" border=0 cellpadding=0 cellspacing=0>
			  ...
			  <tr>
				<td style="padding-bottom:10px;padding-top:10px;">
					...
					Name : <input type="text" name="ntext" id="ntext" value="<?=$tname?>" style="width:180px;margin-right:20px;">
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="108px">Currency Code</th>
										<th width="323px">Currency Name</th>
										<th width="152px">Currency Flag</th>
										<th width="147px"></th>
									</tr>
								</table>
								</div>
...
<?
									$strsql="select * from Tbl_SE_Level";
									$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
