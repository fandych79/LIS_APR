...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM Tbl_BusinessType
   					WHERE btype_code='$thecode'";
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM Tbl_BusinessType
   					WHERE btype_code='$ctext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE Tbl_BusinessType SET btype_name = '$ntext',
											btype_flag = '$ftext',
											btype_active = '$atext',
											btype_create_userid = '$userid',
											btype_create_time = '$dtext'
											where btype_code = '$ctext'";
   }
   else
   {
      $tsql = "INSERT INTO Tbl_BusinessType VALUES('$ctext','$ntext','$userid','$atext','$ftext','$dtext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./Tbl_BusinessType.php";
...
<?
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			$tcode = $row['btype_code'];
			$tname = $row['btype_name'];
			$tflag = $row['btype_flag'];
			$tactive = $row['btype_active'];
		}
	}
	//echo $tcode;
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="100px">Type Code</th>
										<th width="293px">Type Name</th>
										<th width="100px">Type Flag</th>
										<th width="100px">Type Active</th>
										<th width="134px"></th>
									</tr>
								</table>
								</div>
...
<?
									$strsql="select * from Tbl_BusinessType";
									$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
