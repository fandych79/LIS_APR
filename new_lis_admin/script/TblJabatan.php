...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM TblJabatan
   					WHERE jabatan_code='$thecode'";
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM TblJabatan
   					WHERE jabatan_code='$ctext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE TblJabatan SET jabatan_name = '$ntext',
											jabatan_flag = '$ftext',
											payment_active = '$atext',
											jabatan_create_userid = '$userid',
											jabatan_create_time = '$dtext'
											where jabatan_code = '$ctext'";
   }
   else
   {
      $tsql = "INSERT INTO TblJabatan VALUES('$ctext','$ntext','$ftext','$atext','$userid','$dtext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./TblJabatan.php";
...
<?
	...
	$tsql = "SELECT * FROM TblJabatan
					WHERE jabatan_code='$thekey'";
	$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
	$params = array(&$_POST['query']);
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			$tcode = $row['jabatan_code'];
			$tname = $row['jabatan_name'];
			$tflag = $row['jabatan_flag'];
			$tactive = $row['payment_active'];
		}
	}
	//echo $tcode;
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="100px">Jabatan Code</th>
										<th width="293px">Jabatan Name</th>
										<th width="100px">Jabatan Flag</th>
										<th width="100px">Payment Active</th>
										<th width="134px"></th>
									</tr>
								</table>
								</div>
