...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM Tbl_BusinessTerm
   					WHERE bterm_code='$thecode'";
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM Tbl_BusinessTerm
   					WHERE bterm_code='$ctext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE Tbl_BusinessTerm SET bterm_name = '$ntext',
											bterm_flag = '$ftext',
											bterm_active = '$atext',
											bterm_create_userid = '$userid',
											bterm_create_time = '$dtext'
											where bterm_code = '$ctext'";
   }
   else
   {
      $tsql = "INSERT INTO Tbl_BusinessTerm VALUES('$ctext','$ntext','$ftext','$atext','$userid','$dtext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./Tbl_BusinessTerm.php";
...
<?
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			$tcode = $row['bterm_code'];
			$tname = $row['bterm_name'];
			$tflag = $row['bterm_flag'];
			$tactive = $row['bterm_active'];
		}
	}
	//echo $tcode;
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="100px">Term Code</th>
										<th width="293px">Term Name</th>
										<th width="100px">Term Flag</th>
										<th width="100px">Term Active</th>
										<th width="134px"></th>
									</tr>
								</table>
								</div>
...
<?
									$strsql="select * from Tbl_BusinessTerm";
									$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
