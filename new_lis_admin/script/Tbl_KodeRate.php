...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM Tbl_KodeRate
   					WHERE rate_number='$thecode'";
...
function SAVETABLE()
{
  ...
  $userpwd=$_POST['userpwd'];
   ...
   $ftext=$_POST['ftext'];
   $atext=$_POST['atext'];
   $curtext=floatval($_POST['curtext']);
   $destext=$_POST['destext'];
   $dtext=Date('Y-m-d h:m:s');

    
require ("../lib/open_con.php");
   
   $tsql = "SELECT COUNT(*) as b FROM Tbl_KodeRate
   					WHERE rate_number='$ctext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE Tbl_KodeRate SET rate_currency = '$ntext',
											rate_current = '$curtext',
											rate_description = '$destext',
											rate_create_userid = '$userid',
											rate_create_time = '$dtext',
											rate_flag = '$ftext',
											rate_active = '$atext'
											where rate_number = '$ctext'";
   }
   ...
   {
      $tsql = "INSERT INTO Tbl_KodeRate VALUES('$ctext','$ntext','$curtext','$destext','$userid','$dtext','$ftext','$atext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./Tbl_KodeRate.php";
        ...
        function cekSave()
        {
			...
			var vrc = document.formsubmit.curtext;
			var vrd = document.formsubmit.destext;
			
			if(vcode.value.trim().length == 0){
				 alert("Rate Number harus diisi ");
				 vcode.focus();
				 return false;
			}
			else if(vname.value.trim().length == 0){
				 alert("Rate Currency harus diisi ");
				 vname.focus();
				 return false;
			}
			else if(vrc.value.trim().length == 0){
				 alert("Rate Current harus diisi ");
				 vrc.focus();
				 return false;
			}
			else if(vrd.value.trim().length == 0){
				 alert("Rate Description harus diisi ");
				 vrd.focus();
				 return false;
			}
			else{
...
<?
	...
	$tsql = "SELECT * FROM Tbl_KodeRate
					WHERE rate_number='$thekey'";
	$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
	$params = array(&$_POST['query']);
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			$tcode = $row['rate_number'];
			$tname = $row['rate_currency'];
			$tflag = $row['rate_flag'];
			$tactive = $row['rate_active'];
			$tcurrent = $row['rate_current'];
			$tdesc = $row['rate_description'];
		}
	}
	//echo $tcode;
   	      ...
   	      <table width="760" border=0 cellpadding=0 cellspacing=0>
			  ...
			  <tr>
				<td style="padding-bottom:10px;padding-top:10px;">
					Number : <input type="text" name="ctext" id="ctext" value="<?=$tcode?>" style="width:30px;margin-right:20px;">
					Currency : <input type="text" name="ntext" id="ntext" value="<?=$tname?>" style="width:30px;margin-right:20px;">
					Current : <input type="text" name="curtext" id="curtext" value="<?=$tcurrent?>" style="width:50px;margin-right:20px;">
					Description : <input type="text" name="destext" id="destext" value="<?=$tdesc?>" style="width:50px;margin-right:20px;">
					Flag : <input type="text" name="ftext" id="ftext" value="<?=$tflag?>" style="width:20px;margin-right:20px;">
					Active : <input type="text" name="atext" id="atext" value="<?=$tactive?>" style="width:20px;margin-right:20px;">
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="51px">Rate Number</th>
										<th width="103px">Rate Currency</th>
										<th width="72px">Rate Current</th>
										<th width="208px">Rate Description</th>
										<th width="73px">Rate Flag</th>
										<th width="73px">Rate Active</th>
										<th width="141px"></th>
									</tr>
								</table>
								</div>
...
<?
									$strsql="select * from Tbl_KodeRate";
									$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
