...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM Tbl_SE_UserProgram
   					WHERE user_id='$thecode' and program_code = '$theprog'";
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM Tbl_SE_UserProgram
   					WHERE user_id='$ctext' and program_code = '$theprog'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE Tbl_SE_UserProgram SET program_code = '$ntext',
											user_permissions = '$ftext'
											where user_id = '$ctext' and program_code = '$theprog'";
   }
   else
   {
      $tsql = "INSERT INTO Tbl_SE_UserProgram VALUES('$ctext','$ntext','$ftext')";
   }
...
?>
        ...
        function cekDel(thecode,theprog)
        {
           document.formsubmit.target = "utama";
           document.formsubmit.act.value = "deltable";
           document.formsubmit.thecode.value = thecode;
           document.formsubmit.theprog.value = theprog;
           document.formsubmit.action = "./Tbl_SE_UserProgram.php";
		...
		function cekEdit(thekey,theprog)
        {
			//alert(theprog);exit;
           document.formsubmit.target = "utama";
...
<?
	...
	$tsql = "SELECT * FROM Tbl_SE_UserProgram
					WHERE user_id='$thekey' and program_code='$theprog'";
	$cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
	$params = array(&$_POST['query']);
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		{
			$tcode = $row['user_id'];
			$tname = $row['program_code'];
			$tflag = $row['user_permissions'];
		}
	}
	//echo $tcode;
   	      ...
   	      <table width="760" border=0 cellpadding=0 cellspacing=0>
			  ...
			  <tr>
				<td style="padding-bottom:10px;padding-top:10px;">
					ID : <input type="text" name="ctext" id="ctext" value="<?=$tcode?>" style="width:70px;margin-right:20px;">
					Code : <input type="text" name="ntext" id="ntext" value="<?=$tname?>" style="width:180px;margin-right:20px;">
					Permissions : <input type="text" name="ftext" id="ftext" value="<?=$tflag?>" style="width:70px;margin-right:20px;">
					<input type="button" name="btnsave" id="btnsave" value="<?=$bse?>" onclick="cekSave('<?=$theprog?>')">
				</td>
			  </tr>
			  <tr>
				<td width=100% align=left valign=top>
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="117px">ID</th>
										<th width="338px">Code</th>
										<th width="118px">Permissions</th>
										<th width="157px"></th>
									</tr>
								</table>
								</div>
...
?>
									<tr align="center" style="line-height:20px">
										...
										<td><?=$row[2]?></td>
										<td>
											<a href="javascript:cekEdit('<?=$row[0]?>','<?=$row[1]?>');"><img src="../images/icon/edit.png" width="20px" style="border-color:#FFFFFF;" height="20px"></a>
											<a href="javascript:cekDel('<? echo $row[0]?>','<?=$row[1]?>');"><img src="../images/icon/delete.png" style="border-color:#FFFFFF;margin-top:5px;margin-left:10px;" width="20px" height="20px"></a>
										</td>
									</tr>
<?
										}
...
?>
    								 ...
    								 <input type=hidden name=theprog>
