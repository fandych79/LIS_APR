...
require ("../lib/open_con.php");
      ...
      $tsql = "DELETE FROM Tbl_DocCol
   					WHERE doc_id='$thecode'";
...
require ("../lib/open_con.php");
   ...
   $tsql = "SELECT COUNT(*) as b FROM Tbl_DocCol
   					WHERE doc_id='$ctext'";
   $cursorType = array("Scrollable" => SQLSRV_CURSOR_KEYSET);
   $params = array(&$_POST['query']);
   ...
   if ($rowcount > 0)
   {
      $tsql = "UPDATE Tbl_DocCol SET doc_name = '$ntext',
											doc_col_type = '$cttext',
											doc_create_userid = '$userid',
											doc_create_time = '$dtext',
											doc_flag = '$ftext',
											doc_active = '$atext'
											where doc_id = '$ctext'";
   }
   ...
   {
      $tsql = "INSERT INTO Tbl_DocCol VALUES('$ctext','$ntext','$cttext','$userid','$dtext','$ftext','$atext')";
   }
...
?>
        ...
        function cekDel(thecode)
        {
           ...
           document.formsubmit.action = "./Tbl_DocCol.php";
        ...
        function cekSave()
        {
			...
			var vct = document.formsubmit.cttext;
			...
			else if(vct.value.trim().length == 0){
				 alert("Collateral Type harus diisi ");
				 vct.focus();
				 return false;
			}
			else{
...
<?
	...
	if(sqlsrv_has_rows($sqlConn))
	{
		...
		while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
		{
			...
			$tname = $row['doc_name'];
			$tflag = $row['doc_flag'];
			$tactive = $row['doc_active'];
			$tcol = $row['doc_col_type'];
		}
	}
	//echo $tcode;
   	      ...
   	      <table width="760" border=0 cellpadding=0 cellspacing=0>
			  ...
			  <tr>
				<td style="padding-bottom:10px;padding-top:10px;">
					Code : <input type="text" name="ctext" id="ctext" value="<?=$tcode?>" style="width:50px;margin-right:20px;">
					Name : <input type="text" name="ntext" id="ntext" value="<?=$tname?>" style="width:100px;margin-right:20px;">
					Col Type : <input type="text" name="cttext" id="cttext" value="<?=$tcol?>" style="width:70px;margin-right:20px;">
					Flag : <input type="text" name="ftext" id="ftext" value="<?=$tflag?>" style="width:50px;margin-right:20px;">
					Active : <input type="text" name="atext" id="atext" value="<?=$tactive?>" style="width:50px;margin-right:20px;">
							...
							<center>
								...
								<table class="tablein" border="1">
									<tr align="center" height="20px">
										<th width="70px">Doc Code</th>
										<th width="304px">Doc Name</th>
										<th width="71px">Doc Col Type</th>
										<th width="71px">Doc Flag</th>
										<th width="71px">Doc Active</th>
										<th width="137px"></th>
									</tr>
								</table>
								</div>
