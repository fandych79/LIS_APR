...
if (substr($act,0,11) == "viewflow")
{
   ...
   $newbacktoflow=$_POST['newbacktoflow'];
   $newwfid=$_POST['newwfid'];
   if ($utilformfield == "newprevflow")
   {
      $dataall=explode("|",$newprevflow);
   }
   else
   {
      $dataall=explode("|",$newbacktoflow);
   }
   $statuswfid = 0;
...
?>
   	  	      			         								         <option value='<? echo $row2['level_code']; ?>' <? echo $varselected; ?>>[<? echo $row2['level_flag']; ?>]&nbsp;<? echo $row2['level_code']; ?> - <? echo $row2['level_name']; ?></option>
<?
      		   }
   				}
   	      sqlsrv_free_stmt( $sqlConn2 );
...
if ($act == "viewProcCode")
{
  $utilformname=$_POST['utilformname'];
  $utilformfield=$_POST['utilformfield'];
  $check_proc_code=$_POST['check_proc_code'];
  $dataall=explode("||",$check_proc_code);
  $countdataall = 0;
  foreach ($dataall as $t)
  {
    $countdataall++;
...
<?
          ...
          if(sqlsrv_has_rows($sqlConn2))
          {
             ...
             {
?>
                if (document.utilview.T<? echo $row2['proc_code']; ?>.checked == true)
                {
                   varreturn = varreturn + '<? echo $row2['proc_code']; ?>';
                   varreturn = varreturn + '||';
                }
<?
             }
                ...
                <td width=100% align=left valign=top>
                  <form name=utilview method=post>
                     <table width=100% border=0 cellpadding=0 cellspacing=0>
                        <tr>
                          <td width=100% align=center valign=top>
                            <font face=Arial size=2><b>Daftar Proc code</b></font>
                              ...
                              <tr>
                                <td width=100% align=left valign=top>
                                  <table width=100% cellpadding=0 cellspacing=0 border=0>
                                     ...
                                     <tr>
                                        <td width=100% align=left valign=top bgcolor=#006699>
                                          <font face=Arial size=2 color=#FFFFFF><b>&nbsp PROC CODE</b></fomt>
                                        </td>
                                     </tr>
<?
          $tsql2 = "SELECT *
          ...
          if(sqlsrv_has_rows($sqlConn2))
          {
             ...
             while( $row2 = sqlsrv_fetch_array( $sqlConn2, SQLSRV_FETCH_ASSOC))
             {
                ...
                if ($countdataall > 0)
                {
                  for ($zz=0;$zz<$countdataall;$zz++)
                  {
                     if ($row2['proc_code'] == $dataall[$zz])
                     {
                         $varselected = "checked";
                     }
                  }
                                        ...
                                        <td width=100% align=left valign=top>
                                           <table width=100% border=0 cellpadding=0 cellspacing=0>
                                              <tr>
                                                 ...
                                                 <td width=45% align=left valign=top>
<?
                                        print "<input type=checkbox name='T$row2[proc_code]' value='Y' $varselected><font face=Arial size=2>$row2[proc_code] - $row2[proc_name]</font>";
?>
                                                 </td>
                                              </tr>
                                          </table>
