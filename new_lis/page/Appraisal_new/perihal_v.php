...
<html>
	...
	<body>
        <form id="frm" method="post" action="eriksave.php" enctype="multipart/form-data">
            <table align="center" id="tblpreview" border="1" style ="width:1000px; border-color:black;border-collapse:collapse;">
                <tr>
                    ...
                    <!---->
						...
						?>
						<hr>
						<div style="font-weight:bold;text-align:center;"> FORMULIR BERITA ACARA PEMERIKSAAN</div>
						<hr>
						<p>
						Pada tanggal <?=$_tgl?> kami telah melakukan peninjauan setempat jaminan dari debitur dengan data - data sebagai berikut
						</p>
						<!--<table>
							<tr>
								<td>1</td>
...
//                                echo "TIDAK ADA YANG DIAPPRASIAL";
						...
						?>
                            ...
                            <?php
                                $cust_jeniscol="";
                                $url = "./perihal_v.php?custnomid=$custnomid&userwfid=$userwfid&userpermission=$userpermission&buttonaction=$buttonaction&userbranch=$userbranch&userregion=$userregion&userid=$userid&userpwd=$userpwd";
                            ...
                            <?php
                                ...
                                if(sqlsrv_has_rows($sqlConn))
                                {
                                    ...
                                    while( $row = sqlsrv_fetch_array( $sqlConn, SQLSRV_FETCH_ASSOC))
                                    {
										...
										else if($("#_catatan'.$row['_idx'].'").val()=="")
										{
											alert("_catatan '.$no .' harus diisi");
											$("#_catatan'.$row['_idx'].'").focus();
										}
										else if($("#_keterangan'.$row['_idx'].'").val()=="")
										{
											alert("_keterangan '.$no .' harus diisi");
											$("#_keterangan'.$row['_idx'] .'").focus();
										}
										';
										...
										<tr>
											...
											<td style="padding-left:5px;"><?=$_kesiapan;?></td>
											<td style="padding-left:5px;"><?=$_kondisi;?></td>
											<!--<td>
												<?=nl2br($_catatan)?>
											</td>-->
											<td>
